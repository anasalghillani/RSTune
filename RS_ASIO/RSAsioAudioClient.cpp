...
void RSAsioAudioClient::OnAsioBufferSwitch(unsigned buffIdx)
{
	...
	else
	{
		...
		// RSTune sits here, after the ASIO input has been laid into the game's buffer
		// format and before the game's own volume stage, so the game sees the shifted
		// signal for both its tuner and its note detection.
		ProcessRSTune(gameSampleType, gameSampleTypeSize);
...
void RSAsioAudioClient::SetupRSTune()
{
	...
	const unsigned numCh = m_WaveFormat.Format.nChannels;
	const double sr = (double)m_WaveFormat.Format.nSamplesPerSec;
	if (numCh == 0 || m_bufferNumFrames == 0)
	{
		m_TuneEligible = false;
		return;
	}

	m_TuneShifters.resize(numCh);
	for (PitchShifter& ps : m_TuneShifters)
		ps.Init(sr);
...
void RSAsioAudioClient::ProcessRSTune(ASIOSampleType gameSampleType, WORD gameSampleTypeSize)
{
	...
	const WORD stride = m_WaveFormat.Format.nBlockAlign;
	const DWORD frames = m_bufferNumFrames;
	float inPeak = 0.0f, outPeak = 0.0f;

	for (size_t ch = 0; ch < m_TuneShifters.size(); ++ch)
	{
		PitchShifter& ps = m_TuneShifters[ch];
		ps.SetQuality(ctl.quality);
		ps.SetRatio(ratio);

		BYTE* chBase = m_frontBuffer.data() + ch * gameSampleTypeSize;
		...
		if (ch == 0)
		{
			...
			{
				const float a = fabsf(m_TuneScratch[i]);
